...
第 17 節「FileLocker 本體切換消費來源」）。這件事**目前還沒有實作完成**——FileLocker.App 那邊還沒有
改成向 `lx-kvn/PasswordVault` 的 Release 找資產。等那一步做完之後，這個 skill 需要再補上一個步驟：
打包一份給 FileLocker 消費的 zip，命名規則是
...
`FileLocker.PluginContracts.dll` 最後一次更新時間點手動決定，不是自動算出來的。**在 FileLocker 那邊的
切換完成之前，這個 zip 還不需要打包**，這個 skill 目前只涵蓋 PasswordVault 自己的 GUI 安裝程式發布；
等切換完成回頭補這個步驟時，記得同步更新這份 SKILL.md。
...
2. **跑完整測試套件**：`dotnet test PasswordVault.slnx`。任何一個測試沒過就停下來，不要 continue——
   回報給使用者，不要自己決定要不要跳過。**目前 `tests/` 底下只有 `PasswordVault.Core.Tests`**（見
   README「尚未完成」段落），`PasswordVault.App`／`PasswordVault.Cli` 還沒有對應測試專案，這不是這個
   skill 要處理的事，只是提醒這一步驗證範圍目前有限。
3. **Release 組態建置**：`dotnet build PasswordVault.slnx -c Release`，確認 0 錯誤 0 警告。
4. **決定版本號**：第一次發布固定 `0.1.0`；之後每次讀 `git tag` 列出目前最新版本，跟使用者確認這次是
   patch／minor／major，不要自己猜。
5. **產生 Release Notes 草稿**：第一次發布沒有「上一個 tag」可以 diff，改成整理這個 repo從
   `1de9f73 docs: 初始化 PasswordVault repo` 以來的完整變更（`git log --oneline`），依照 FileLocker
   `docs/releases/v1.1.0.md` 的雙語段落結構寫成 `docs/releases/v0.1.0.md`（亮點／已知限制，中文在前、
   英文在後）。之後每次發布才有真正的「上一個 tag」可以 `git log <上一個tag>..HEAD --oneline` 整理。
   同時檢查 README.md「已完成」／「尚未完成」段落需不需要跟著更新。
...
- 不打包給 FileLocker 消費的相容性 zip——這個功能還沒實作完成，見上方「額外要注意」段落。
